...
def main():
    ...
    testset_name = 'set5'  # set test set,  'set5' | 'srbsd68'
    ...
    for sf in test_sf:
        ...
        for k_index in range(k_num):
            ...
            for idx, img in enumerate(L_paths):
                ...
                logger.info(
                    '{:->4d}--> {:>10s} -- sf:{:>1d} --k:{:>2d} PSNR: {:.2f}dB'.format(idx + 1, img_name + ext, sf,
                                                                                       k_index, psnr))
            ...
            print(
                '------> Average PSNR(RGB) of ({}) scale factor: ({}), kernel: ({}) sigma: ({:.2f}): {:.2f} dB'.format(
    ...
    print('------> Average PSNR of ({}) {:.2f} dB'.format(testset_name, ave_psnr_sf_k))
    if n_channels == 3:
        ave_psnr_y_sf_k = sum(test_results_ave['psnr_y_sf_k']) / len(test_results_ave['psnr_y_sf_k'])
        print('------> Average PSNR of ({}) {:.2f} dB'.format(testset_name, ave_psnr_y_sf_k))
